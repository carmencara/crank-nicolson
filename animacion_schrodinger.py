...
# Parámetros
# ========================================
file_in = "funcion_onda.txt" # Nombre del fichero de datos
file_out = "evolucion" # Nombre del fichero de salida (sin extensión)
...
#data1 = np.loadtxt('V-pared-barranco.txt')
data1 = np.loadtxt('V-barrera-pozo.txt')
pot = data1[:,1]
pos = data1[:,0]
ax.plot(pos, pot, linestyle='-', color='#EC118C')

# Plotear el título de la gráfica
#ax.set_title("Paquete de ondas gaussiano con $k_0=500$ sometido a $V(x)=0$,\n con $\\sigma^2=10^{-3}$, $\\Delta x=10^{-4}$, $\\Delta t=5\cdot 10^{-7}$ y 4000 iteraciones", fontsize='10')
ax.set_title("Barrera de potencial", fontsize='10')
# Nombres de los ejes
ax.set_xlabel("$x$")
